...
func TestReactionRetry429ThenSuccess(t *testing.T) {
	var calls int32
	...
	if payloads[0]["emotions"]["key"] != payloads[1]["emotions"]["key"] {
		t.Fatalf("emotion key changed between retries")
	}
	if payloads[0]["emotions"]["value"].(float64) != payloads[1]["emotions"]["value"].(float64) {
		t.Fatalf("emotion value changed between retries")
	}
}

func TestReactionRetry429Exhausted(t *testing.T) {
	var calls int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestReactionRetry500(t *testing.T) {
	var calls int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		call := atomic.AddInt32(&calls, 1)
		if call == 1 {
	...
	status, err := client.AddReaction(context.Background(), "@19:abc@thread.v2", "msg/1", "like", 1)
	if err != nil {
...
func TestReactionNoRetryOn400(t *testing.T) {
	var calls int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		atomic.AddInt32(&calls, 1)
		w.WriteHeader(http.StatusBadRequest)
...
func TestReactionContextCancelStopsRetries(t *testing.T) {
	var calls int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		atomic.AddInt32(&calls, 1)
		w.WriteHeader(http.StatusBadGateway)
	...
	_, err := client.AddReaction(ctx, "@19:abc@thread.v2", "msg/1", "like", 1)
